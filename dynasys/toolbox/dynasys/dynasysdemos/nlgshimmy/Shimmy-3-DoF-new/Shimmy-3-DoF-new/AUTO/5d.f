...
C
	...
	ALPHAG=10.0D0*3.1415926/180.0D0
        EEFF=PAR(1)*COS(PAR(8))+(0.362D0+PAR(1)
     +      *SIN(PAR(8)))*TAN(PAR(8))
	HLG=2.50D0
C
...
C
	MDDELTA=-PAR(10)*U(4)
C
        MKLAMDELTA=-FY*COS(U(1)*COS(PAR(8)))
     +      *HLG*COS(PAR(8))*COS(U(3))
C
       F(1)=U(2)
     ...
     +      +PAR(4)*EEFF*SIN(U(1)*COS(PAR(8)))) 	
       F(5)=PAR(2)*SIN(U(1)*COS(PAR(8)))+(EEFF-PAR(5))
     +      *U(2)*COS(U(1)*COS(PAR(8)))*COS(PAR(8))
