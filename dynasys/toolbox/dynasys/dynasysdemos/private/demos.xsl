...
$Revision: 1.1.6.23 $  $Date: 2008/02/29 12:45:40 $
-->
...
<xsl:variable name="language" select="locale:getLanguage(locale:getDefault())"/>
...
<head>
  <title><xsl:value-of select="$productName"/> Demos</title>
  <base href="{$demosrooturl}/"/>
  <meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1" />
  <link rel="stylesheet" type="text/css"
    href="{$urlbase}{$matlabroot}/toolbox/matlab/demos/private/demoindex.css"/>
</head>
  ...
  <div id="header-container">
    <div id="header">
      <div>
        ...
        <span class="header-demos">DEMOS</span>
      </div>
        ...
        <xsl:choose>
          ...
          <xsl:otherwise>
            ...
            <a href="http://www.mathworks.com/pl_{$uniqueProductDir}">
                <img src="{$urlbase}{$matlabroot}/toolbox/matlab/demos/private/demo_arrow.png" border="none" alt="arrow" align="middle" />
            </a>
          </xsl:otherwise>
        </xsl:choose>
      </div>
  ...
  <xsl:otherwise>
    ...
    <xsl:choose>
      <xsl:when test="$subsection">
        <xsl:apply-templates select="//demosection[label=$section]/demosection[label=$subsection]"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:apply-templates select="//demosection[label=$section]"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:otherwise>
</xsl:choose>
  ...
  <!-- section heading -->
    <xsl:choose>
      ...
      <xsl:when test="count(ancestor::*)=2">
        <!-- Sub-section -->
        <div class="demopanel-sub-container">
  ...
  <xsl:variable name="link">
    ...
    <xsl:choose>
      <xsl:when test="type and file">
          <xsl:variable name="localFile"><xsl:call-template name="buildLocalizedPath">
      ...
      <xsl:when test="'M-file'=type">
        ...
        <xsl:variable name="localFile" select="concat('html/',$language,'/',$source,'.html')" />
      ...
      <td width="80" class="demopanel-type" align="left">
        <xsl:choose>
          <xsl:when test="type='M-file'">
            <img src="{$urlbase}{$matlabroot}/toolbox/matlab/demos/private/type_m-file.png" alt="{$mfiledesc}" align="middle"/><xsl:value-of select="$mfile"/>
          </xsl:when>
          <xsl:when test="type='M-GUI'">
            <img src="{$urlbase}{$matlabroot}/toolbox/matlab/demos/private/type_m-gui.png" alt="{$mguidesc}" align="middle"/><xsl:value-of select="$mgui"/>
          </xsl:when>
          <xsl:when test="type='model'">
            <img src="{$urlbase}{$matlabroot}/toolbox/matlab/demos/private/type_model.png" alt="{$modeldesc}" align="middle"/><xsl:value-of select="$model"/>
          </xsl:when>
          <xsl:when test="type='video'">
             <img src="{$urlbase}{$matlabroot}/toolbox/matlab/demos/private/type_video.png" alt="{$videodesc}" align="middle"/><xsl:value-of select="$video"/>
          </xsl:when>
        </xsl:choose>
      </td>
    </tr>
...
<xsl:template name="chooseThumbnail">
  ...
  <xsl:choose>
    ...
    <xsl:when test="type='M-file'"><xsl:value-of select="$matlabroot"/>/toolbox/matlab/demos/private/type_m-file_lg.png</xsl:when>
    <xsl:when test="type='video'"><xsl:value-of select="$matlabroot"/>/toolbox/matlab/demos/private/type_video_lg.png</xsl:when>
...
<xsl:template name="buildLocalizedPath">
   ...
   <xsl:choose>
      ...
      <xsl:otherwise><xsl:value-of select="$language" />/<xsl:value-of select="$filepath" /></xsl:otherwise>
   </xsl:choose>
</xsl:template>
