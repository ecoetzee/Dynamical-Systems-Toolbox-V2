...
!
       IF(IID.GT.4) THEN
           ...
           DO I=1,NDIM
               IF(FORT9DST==1)WRITE(9,104)(C0(I,J),J=1,NDIM)
           ENDDO
           IF(FORT9DST==1)WRITE(9,103)
           DO I=1,NDIM
               IF(FORT9DST==1)WRITE(9,104)(C1(I,J),J=1,NDIM)
           ENDDO
       END IF
!
!  PART I:
...
!
       IF(IID.GT.4) THEN
           ...
           IF(FORT9DST==1)WRITE(9,106)
           DO I=1,NDIM
               IF(FORT9DST==1)WRITE(9,104)(C0(I,J),J=1,NDIM)
           ENDDO
           IF(FORT9DST==1)WRITE(9,107)
           DO I=1,NDIM
               IF(FORT9DST==1)WRITE(9,104)(C1(I,J),J=1,NDIM)
            ENDDO
       END IF
...
!
	  ...
	  CALL AUTOSTOPWITHERROR('Domain error for K in DHHPR')
      END IF
      IF (J .GT. N) THEN
	  WRITE (9,*) 'Domain error for J in DHHPR'
	  CALL AUTOSTOPWITHERROR('Domain error for J in DHHPR')
      END IF
      IF (INCX .LT. 1) THEN
	  WRITE (9,*) 'Domain error for INCX in DHHPR'
	  CALL AUTOSTOPWITHERROR('Domain error for INCX in DHHPR')
      END IF
!
...
!
	  ...
	  CALL AUTOSTOPWITHERROR('Domain error for JOB in DHHAP')
      END IF
      IF ((K .LT. 1) .OR. (K .GT. J)) THEN
	  WRITE (9,*) 'Domain error for K in DHHAP'
	  CALL AUTOSTOPWITHERROR('Domain error for K in DHHAP')
      END IF
      IF (JOB .EQ. 1) THEN
          IF (J .GT. N) THEN
	      WRITE (9,*) 'Domain error for J in DHHAP'
	      CALL AUTOSTOPWITHERROR('Domain error for J in DHHAP')
          END IF
      ELSE
          IF (J .GT. Q) THEN
	      WRITE (9,*) 'Domain error for J in DHHAP'
